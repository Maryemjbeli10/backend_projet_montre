...
Objectif :
Développer une application intelligente qui aide un utilisateur à décider s’il doit acheter une montre comme investissement, en s’appuyant sur :
Des données historiques (fichier CSV watches.csv),
...
Marque (Rolex, Omega…) , Modèle , Année de production , Prix d’achat , État .
...
Moyen
Risqué
Explication de la prédiction (XAI) à l’aide de :
SHAP (identification des variables les plus influentes sur les prédictions globales du modèle.),
